...
copyright    = '1996-2010 Robin Mills'
prev         = default + ext
lightbox     = 'lightbox'
download     = '2.35mb'
...
def syntax():
    ...
    print "syntax: webby [[-photos] <photoDirectory>] [-config config] [-webdir <webdir>] [-title <title>]]+ "
...
def geotagIcon(jpeg,pos):
	icon = os.path.expanduser('~/bin/geotag-128.jpg')
...
def cmpDate(x,y):          return compare(filedict[x][1],filedict[y][1])
...
#   print "filesInDirectory => ",filesInDirectory
    ...
    if os.path.samefile(os.path.abspath(myData),os.path.abspath(directoryName)):
...
def writeImages(webDir,filename,pathname,width,aspect,rotate,cols,bGeotagIcon):
    """writeImages - creat images and thumbnails"""
    # write the image
...
#            + ' "' + pathname + '" --out "' + imagename + '"'
            ...
            W = float(width) if aspect < 1.0 else float(width)/(aspect*aspect)
            W = int ( W / cols )
            cmd  = 'sips '
            if rotate:
                cmd += ' --rotate ' + str(rotate)
...
#   print sys.path 
    ...
    if os.path.samefile(mepath,configModule.__file__):
    ...
    if 1:   
        ...
        for k in sorted(filedict.keys(),cmpDate):
            ...
            try:
                ...
                if image['Exif.Image.Orientation'] == 6:
                    rotate = 90

                latr= str(image['Exif.GPSInfo.GPSLatitudeRef'   ])
                lonr= str(image['Exif.GPSInfo.GPSLongitudeRef'  ])
                altr= int(image['Exif.GPSInfo.GPSAltitudeRef'   ])

                lat = ff (image['Exif.GPSInfo.GPSLatitude'      ])
                lon = ff (image['Exif.GPSInfo.GPSLongitude'     ])
                las = fs (image['Exif.GPSInfo.GPSLatitude'      ])
                los = fs (image['Exif.GPSInfo.GPSLongitude'     ])

                alt = f  (image['Exif.GPSInfo.GPSAltitude'      ])
...
#           print timestring, "=> ",filename
            ...
            thumbs         += '  <td><center><a href="' + subs['iname'] + '"><img src="' + e + '" longdesc="' + I + '"></a><br>' + subs['pname']  + '</center></td>\n'
            thumbsdiv      += '  <td><center><div class=boxshadow><div class=boxmain><a href="' + subs['iname'] + '"><img src="' + e + '" class="thumb"></a><br>' + subs['pname']  + '</div></div></center></td>\n'
