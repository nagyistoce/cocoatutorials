...
void QuickTimeVideo::tagDecoder(Exiv2::DataBuf &buf, uint32_t size){
    ...
    if (ignoreList(buf)) discard(size);
    else if (dataIgnoreList(buf)) decodeBlock();
    else if(decodetIter !=  NULL) (this->*decodetIter)(size);
    ...
    // Decoders with lesser metadata
        ...
        if(!d->m_modifyMetadata){
            ...
            else if (d->currentStream_ == Audio) xmpData_["Xmp.audio.URN"] = Exiv2::toString(buf.pData_);
        }
        else{
...
void QuickTimeVideo::timeToSampleDecoder(uint32_t iDummySize){
    ...
    if(!d->m_modifyMetadata){
        if (d->currentStream_ == Video) xmpData_["Xmp.video.FrameRate"] = (double)totalframes * (double)d->timeScale_ / (double)timeOfFrames;
        else
...
void QuickTimeVideo::audioDescDecoder(){
    ...
    if(!d->m_modifyMetadata){
        ...
        for (i = 0; size/4 != 0 ; size -= 4, i++){
            io_->read(buf.pData_, 4);
            switch(i){
            case AudioFormat: td = find(qTimeFileType, Exiv2::toString( buf.pData_));
                if(td) xmpData_["Xmp.audio.Compressor"] = exvGettext(td->label_);
                else xmpData_["Xmp.audio.Compressor"] = Exiv2::toString( buf.pData_);
                break;

            case AudioVendorID: td = find(vendorIDTags, Exiv2::toString( buf.pData_));
                if(td) xmpData_["Xmp.audio.VendorID"] = exvGettext(td->label_);
                break;

            case AudioChannels:
                xmpData_["Xmp.audio.ChannelType"] = returnBufValue(buf, 2);
                xmpData_["Xmp.audio.BitsPerSample"] = (buf.pData_[2] * 256 + buf.pData_[3]); break;

            case AudioSampleRate: xmpData_["Xmp.audio.SampleRate"] = returnBufValue(buf, 2) + ((buf.pData_[2] * 256 + buf.pData_[3]) * 0.01); break;
        ...
        for (i = 0; size/4 != 0 ; size -= 4, i++){
            ...
            default: io_->seek(4,BasicIo::cur);
                bDataWriten = true; break;
            }
...
void QuickTimeVideo::imageDescDecoder()
{
    ...
    if(!d->m_modifyMetadata){
        for (int32_t i = 0; size/4 != 0 ; size -= 4, i++){
            ...
            switch(i){
            case codec: td = find(qTimeFileType, Exiv2::toString( buf.pData_));
            ...
            case VendorID: td = find(vendorIDTags, Exiv2::toString( buf.pData_));
                ...
                break;
            case SourceImageWidth_Height: xmpData_["Xmp.video.SourceImageWidth"] = returnBufValue(buf, 2);
                xmpData_["Xmp.video.SourceImageHeight"] = (buf.pData_[2] * 256 + buf.pData_[3]); break;
            case XResolution: xmpData_["Xmp.video.XResolution"] = returnBufValue(buf, 2) + ((buf.pData_[2] * 256 + buf.pData_[3]) * 0.01); break;
            case YResolution: xmpData_["Xmp.video.YResolution"] = returnBufValue(buf, 2) + ((buf.pData_[2] * 256 + buf.pData_[3]) * 0.01);
                io_->read(buf.pData_, 3); size -= 3; break;
            case CompressorName: io_->read(buf.pData_, 32); size -= 32;
                xmpData_["Xmp.video.Compressor"] = Exiv2::toString( buf.pData_); break;
            default: break;
            }
        }
...
void QuickTimeVideo::multipleEntriesDecoder(uint32_t iDummySize){
    ...
    uint32_t i;
    for( i = 1; i <= noOfEntries; i++) decodeBlock();
    return;
...
void QuickTimeVideo::videoHeaderDecoder(uint32_t size){
    ...
    if(!d->m_modifyMetadata){
        ...
        for (i = 0; size/2 != 0 ; size -= 2, i++){
            ...
            switch(i){
            case GraphicsMode: td = find(graphicsModetags, returnBufValue(buf,2));
                if(td) xmpData_["Xmp.video.GraphicsMode"] = exvGettext(td->label_);
                break;
            case OpColor: xmpData_["Xmp.video.OpColor"] = returnBufValue(buf,2); break;
            default: break;
            }
        }
...
void QuickTimeVideo::fileTypeDecoder(uint32_t size)
{
    ...
    if(!d->m_modifyMetadata){
        ...
        for (int32_t i = 0; size/4 != 0; size -=4, i++){
            bool bDataWritten = false;
            switch(i){
            case 0: if(xmpData_["Xmp.video.MajorBrand"].count() > 0){
                ...
                break;
            case 1: if(xmpData_["Xmp.video.MinorVersion"].count() >0){
                    buf = returnBuf((int64_t)xmpData_["Xmp.video.MinorVersion"].toLong());
                    bDataWritten = writeMultibyte(buf.pData_,4);
                }
                break;
            default: if(xmpData_["Xmp.video.CompatibleBrands"].count() >0){
...
void QuickTimeVideo::mediaHeaderDecoder(uint32_t size){
    ...
    if(!d->m_modifyMetadata){
        for (int32_t i = 0; size/4 != 0 ; size -=4, i++){
            ...
            switch(i){
            case MediaHeaderVersion: if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaHeaderVersion"] = returnBufValue(buf,1);
                else if (d->currentStream_ == Audio) xmpData_["Xmp.audio.MediaHeaderVersion"] = returnBufValue(buf,1);
                break;
            case MediaCreateDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaCreateDate"] = returnUnsignedBufValue(buf);
                else if (d->currentStream_ == Audio) xmpData_["Xmp.audio.MediaCreateDate"] = returnUnsignedBufValue(buf);
                break;
            case MediaModifyDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaModifyDate"] = returnUnsignedBufValue(buf);
                else if (d->currentStream_ == Audio) xmpData_["Xmp.audio.MediaModifyDate"] = returnUnsignedBufValue(buf);
                break;
            case MediaTimeScale: if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaTimeScale"] = returnBufValue(buf);
                else if (d->currentStream_ == Audio) xmpData_["Xmp.audio.MediaTimeScale"] = returnBufValue(buf);
                time_scale = returnBufValue(buf);
                break;
            case MediaDuration: if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaDuration"] = returnBufValue(buf)/time_scale;
        ...
        for (int32_t i = 0; size/4 != 0 ; size -=4, i++){
            bool bDataWritten = false;
            switch(i){
            case MediaHeaderVersion: bDataWritten = writeAudVidData("MediaHeaderVersion", 1, 1, 3, 0); break;
            ...
            case MediaTimeScale: bDataWritten = writeAudVidData("MediaTimeScale", 1, 4, -4, 0);
                bDataWritten = true; break;
            case MediaDuration:if(d->currentStream_ == Video){
                    if(xmpData_["Xmp.video.MediaDuration"].count() > 0){
                        io_->read(buf.pData_,4);
                        time_scale = returnBufValue(buf);
                        bDataWritten = writeAudVidData("MediaDuration", time_scale, 4, 0, 0);
                    }
                    else{
                        io_->read(buf.pData_,4);
                        time_scale = returnBufValue(buf);
                ...
                break;
            case MediaLanguageCode: if(d->currentStream_ == Video){
...
void QuickTimeVideo::trackHeaderDecoder(uint32_t size)
{
    ...
    if(!d->m_modifyMetadata){
        for (int32_t i = 0; size/4 != 0  ; size -=4, i++){
            ...
            switch(i){
            case TrackHeaderVersion: if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackHeaderVersion"] = returnBufValue(buf,1);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackHeaderVersion"] = returnBufValue(buf,1);
                break;
            case TrackCreateDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackCreateDate"] = returnUnsignedBufValue(buf);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackCreateDate"] = returnUnsignedBufValue(buf);
                break;
            case TrackModifyDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackModifyDate"] = returnUnsignedBufValue(buf);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackModifyDate"] = returnUnsignedBufValue(buf);
                break;
            case TrackID: if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackID"] = returnBufValue(buf);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackID"] = returnBufValue(buf);
                break;
            case TrackDuration: if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackDuration"] = (int64_t)((double)returnBufValue(buf)
                                                                                                               *1000/(double)d->timeScale_);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackDuration"] = (int64_t)((double)returnBufValue(buf)
                                                                                                    *1000/(double)d->timeScale_);
                break;
            case TrackLayer: if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackLayer"] = returnBufValue(buf, 2);
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackLayer"] = returnBufValue(buf, 2);
                break;
            case TrackVolume: if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackVolume"] = (returnBufValue(buf, 2)/2);//128=100%;
                else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackVolume"] = (returnBufValue(buf, 2)/2);//;
                break;
            case ImageWidth: if(d->currentStream_ == Video){
                    temp = (returnBufValue(buf, 4)/(256*256));
                    xmpData_["Xmp.video.Width"] = temp;
                    d->width_ = temp;
                }
                break;
            case ImageHeight: if(d->currentStream_ == Video){
                    temp = (returnBufValue(buf, 4)/(256*256));
        ...
        for (int32_t i = 0; size/4 != 0  ; size -=4, i++){
            ...
            case ImageWidth:if(d->currentStream_ == Video)  bDataWritten = writeAudVidData("Width", (256*256), 4, 0, 0); break;
            case ImageHeight:if(d->currentStream_ == Video)  bDataWritten = writeAudVidData("Height", (256*256), 4, 0, 0); break;
            default: break;
            }
            if(!bDataWritten) io_->seek(4,BasicIo::cur);
        }
...
void QuickTimeVideo::movieHeaderDecoder(uint32_t size){
    ...
    if(!d->m_modifyMetadata){
        for (int32_t i = 0; size/4 != 0 ; size -=4, i++){
            ...
            case CreateDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                xmpData_["Xmp.video.DateUTC"] = returnUnsignedBufValue(buf); break;
            case ModifyDate: //A 32-bit integer that specifies (in seconds since midnight, January 1, 1904) when the movie atom was created.
                xmpData_["Xmp.video.ModificationDate"] = returnUnsignedBufValue(buf); break;
            case TimeScale: xmpData_["Xmp.video.TimeScale"] = returnBufValue(buf);
        ...
        for (int32_t i = 0; size/4 != 0 ; size -=4, i++){
            ...
            case TimeScale: if(xmpData_["Xmp.video.TimeScale"].count() > 0){
                ...
                break;
            case Duration: if(xmpData_["Xmp.video.Duration"].count() > 0){
                ...
                break;
            case PreferredRate: if(xmpData_["Xmp.video.PreferredRate"].count() > 0){
                    buf = returnBuf((int64_t)xmpData_["Xmp.video.PreferredRate"].toLong()
                            *(256*256),4);
                    bDataWritten = writeMultibyte(buf.pData_,4);
                }
                break;
            case PreferredVolume: if(xmpData_["Xmp.video.PreferredVolume"].count() > 0){
                ...
                break;
            case PreviewTime: if(xmpData_["Xmp.video.PreviewTime"].count() > 0){
                ...
                break;
            case PreviewDuration: if(xmpData_["Xmp.video.PreviewDuration"].count() > 0){
                ...
                break;
            case PosterTime: if(xmpData_["Xmp.video.PosterTime"].count() > 0){
                    int64_t posterTime  = (int64_t)xmpData_["Xmp.video.PosterTime"].toLong();
                    buf = returnBuf(posterTime);
                    bDataWritten = writeMultibyte(buf.pData_,4);
                }
                break;
            case SelectionTime: if(xmpData_["Xmp.video.SelectionTime"].count() > 0){
                    int64_t selectionTime  = (int64_t)xmpData_["Xmp.video.SelectionTime"].toLong();
                    buf = returnBuf(selectionTime);
                    bDataWritten = writeMultibyte(buf.pData_,4);
                }
                break;
            case SelectionDuration: if(xmpData_["Xmp.video.SelectionDuration"].count() > 0){
                ...
                break;
            case CurrentTime: if(xmpData_["Xmp.video.CurrentTime"].count() > 0){
                    int64_t currentTime  = (int64_t)xmpData_["Xmp.video.CurrentTime"].toLong();
                    buf = returnBuf(currentTime);
                    bDataWritten = writeMultibyte(buf.pData_,4);
                }
                break;
            case NextTrackID: if(xmpData_["Xmp.video.NextTrackID"].count() > 0){
