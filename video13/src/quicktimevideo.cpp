		...
		for (int32_t i = 0; size/4 != 0 ; size -=4, i++){
			...
			switch(i){
				case MediaHeaderVersion:
					 if(d->currentStream_ == Video) xmpData_["Xmp.video.MediaHeaderVersion"] = returnBufValue(buf,1);
		...
		for (int32_t i = 0; size/4 != 0	 ; size -=4, i++){
			...
			switch(i){
				case TrackHeaderVersion:
					 if		(d->currentStream_ == Video) xmpData_["Xmp.video.TrackHeaderVersion"] = returnBufValue(buf,1);
				...
				case TrackID:
					if	   (d->currentStream_ == Video) xmpData_["Xmp.video.TrackID"] = returnBufValue(buf);
					else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackID"] = returnBufValue(buf);
				break;
				
				case TrackDuration:
					 if(d->currentStream_ == Video) xmpData_["Xmp.video.TrackDuration"] = (int64_t)((double)returnBufValue(buf)
				...
				case TrackLayer:
					if	   (d->currentStream_ == Video) xmpData_["Xmp.video.TrackLayer"] = returnBufValue(buf, 2);
					else if(d->currentStream_ == Audio) xmpData_["Xmp.audio.TrackLayer"] = returnBufValue(buf, 2);
				break;
				
				case TrackVolume:
					 if		(d->currentStream_ == Video) xmpData_["Xmp.video.TrackVolume"] = (returnBufValue(buf, 2)/2);//128=100%;
